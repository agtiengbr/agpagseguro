...
class CreatePixOrder
{
    ...
    public function __construct(CreateOrder $infraService, GetOrder $getOrder, EntityManagerInterface $em, Configuration $config)
    {
    ...
    public function exec(\Cart $cart, $notificationUrl)
    {
        ...
        if ($r instanceof CreateOrderResponseSuccess) {
            //consulta o objeto Order
            $order = $this->getOrder->exec($r->getOrder());
            $this->postApiRequest($this->getOrder->getRequest(), $this->em);


            $transaction = new AgpagseguroTransaction;
            $transaction->setTransactionReference($r->getOrder()->getReferenceId())
                // ->setTransactionCode($r->getOrder()->getQrCodes()[0]->getId())
                ...
                ->setValue($r->getOrder()->getQrCodes()[0]->getAmount()->getValue())
                ->setExpirationDate($r->getOrder()->getQrCodes()[0]->getExpirationDate())
                ->setTransactionLink($r->getOrder()->getQrCodes()[0]->getLinks()[0]['href'])
                ->setCopyText($r->getOrder()->getQrCodes()[0]->getText())
